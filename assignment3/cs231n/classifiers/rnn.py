...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type is "rnn":
            hidden, rnn_cache = rnn_forward(word_out, affine_out, Wx, Wh, b)
       
        # 4
        temporal_affine_out, temporal_affine_cache = temporal_affine_forward(hidden, W_vocab, b_vocab)
        # 5
        loss, dout = temporal_softmax_loss(temporal_affine_out, captions_out, mask)
        
        #4
        dout_temporal, grads["W_vocab"], grads["b_vocab"]= temporal_affine_backward(dout, temporal_affine_cache)
        #3
        if self.cell_type is "rnn":
            dout_rnn_word, dout_rnn_image, grads["Wx"], grads["Wh"], grads["b"] = rnn_backward(dout_temporal, rnn_cache)
        
            
        #2
        grads["W_embed"]= word_embedding_backward(dout_rnn_word, word_cache)
        
        #1
        _, grads["W_proj"], grads["b_proj"]= affine_backward(dout_rnn_image, affine_cache)
